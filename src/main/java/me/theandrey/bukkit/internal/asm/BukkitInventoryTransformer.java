...
public class BukkitInventoryTransformer implements IClassTransformer {
	...
	private static byte[] patchThaumcraftEntityInventory(String name, byte[] bytes, String entClass) {
		...
		addGetHolderEntity(node, "ent", ASMHelper.getObjectType(entClass));
	...
	private static void addTransactionMethods(ClassNode node) {
		final Type craftHuman = ASMHelper.getObjectType(ASMHelper.getCraftPackage() + ".entity.CraftHumanEntity");
	...
	private static void addGetHolderEntity(ClassNode node, String field, Type entDesc) {
		final Type invHolder = ASMHelper.getObjectType("org.bukkit.inventory.InventoryHolder");
		final Type craftEntity = ASMHelper.getObjectType(ASMHelper.getCraftPackage() + ".entity.CraftEntity");

		MethodVisitor mv = node.visitMethod(Opcodes.ACC_PUBLIC, "getOwner", Type.getMethodDescriptor(invHolder), null, null);
		mv.visitCode();
		mv.visitVarInsn(Opcodes.ALOAD, 0);
		if (field != null) {
			mv.visitFieldInsn(Opcodes.GETFIELD, node.name, field, entDesc.getDescriptor());
		}
		mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, entDesc.getInternalName(), "getBukkitEntity", Type.getMethodDescriptor(craftEntity), false);
